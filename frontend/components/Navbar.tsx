      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-20 items-center">
          <Link to="/" className="flex items-center space-x-3 group">
            ...
            <span className="text-xl font-extrabold tracking-tight text-slate-900 dark:text-white uppercase">PRANIKOV<span className="text-brand-orange">.</span></span>
          </Link>
          ...
          <div className="hidden md:flex items-center space-x-8">
            {links.map((link) => (
              <Link
                ...
                className={`text-sm font-semibold transition-colors hover:text-brand-orange ${
                  location.pathname === link.path ? 'text-brand-orange' : 'text-slate-600 dark:text-slate-400'
                }`}
              >
                {link.name}
              </Link>
            ...
            >
              Start Project <ArrowRight className="ml-2 group-hover:translate-x-1 transition-transform" size={16} />
            </Link>
          </div>
